...
private:
  ...
  /* Bytes and header bytes counters for the forward/backward direction of the
   * flow */
  ...
  /* Count of packets with at least 1 byte of TCP data payload in the forward
   * direction */
  ...
  /*
      ...
      - total bytes of payload (flow and forward/backward direction of the
     flow).
  */
  intAcc flow_iat;
  intAcc forward_iat;
...
void check_connections(Packet *p) {
  ...
  for (auto it = active_connections.begin(); it != active_connections.end();
    ...
    /*
        ...
        that the caller is a thread checking for timeouts in the map of
       currently active connections. In this case, we must compare the current
       time (time(nullptr)) with the last time the target flow was seen. In the
       scenario where we're reading a .pcap, this function will always be called
       with an argument (the current packet being processed), hence we just need
       to compare the packet's time with the last time the target flow was seen.
    */
    if (p == nullptr) {
